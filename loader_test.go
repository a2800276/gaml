...
const test_dir = "./test"
const simple_gaml_fn = "simple.gaml"
...
func TestLoadFile(t *testing.T) {
	...
	if _, err = fsl.Load(simple_gaml_fn); err != nil {
		t.Errorf("couldn't load: %s: %s", simple_gaml_fn, err)
	}
	...
	if httpHandler, err := NewHamlHandler(test_dir); err != nil {
		...
		if expected, err2 := readFile(t, test_dir+"/"+simple_html_fn); err2 != nil {
			...
			if !bytes.Equal(writer.b.Bytes(), expected) {
				t.Errorf("unexpected result. got: >%s< expected: >%s<", writer.b.Bytes(), expected)
			}
		}
		writer.h = nil
